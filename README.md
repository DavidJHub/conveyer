...
| `05_funnel_audit_lab.ipynb` | the audit lab: the conversations parquet reconnected to `fact_scraped_page`, feature-behaviour diagnostics, three independent funnel-stage checks, and the per-conversation journey roadmap with every audit detail on each node |
| `06_scrape_diagnostics.ipynb` | scraper & classifier forensics from an already-scraped parquet (`results/` or `CONVEYER_RUN_DIR`): fetch/evidence health, category·subtype·method breakdowns, the confidence-metric question answered (`page_category_confidence` saturates — triage runs on `suspicion`), and the every-URL diagnostics dashboard |
